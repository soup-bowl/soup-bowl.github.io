...
export const ListingItem = ({
	...
	return (
		<Item>
			{image !== undefined ? <Image style={{ backgroundImage: `url(${image})` }} /> : null}
			<Label>
				<h2>
					<a href={url}>{title}</a>
				</h2>
				<InfoBites>
					{date ? (
						<ItemInfo icon={faClock} alt="Created">
							{date.toLocaleDateString("en-GB")}
						</ItemInfo>
					) : null}
					{daysSince ? (
						<ItemInfo icon={faPenToSquare} alt="Last change">
							{timeSince(daysSince)}
						</ItemInfo>
					) : null}
					{stars > 0 ? (
						<ItemInfo icon={faStar} alt="Stars">
							{stars}
						</ItemInfo>
					) : null}
					{downloads > 0 ? (
						<ItemInfo icon={faDownload} alt="Downloads">
